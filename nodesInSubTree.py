...
def trasverseTree(node, c, s):
    numOfOcurrences = 0
    processNodes = [node]
    while processNodes:
        curNode = processNodes.pop(0)
        if s[curNode.val-1] == c:
            numOfOcurrences += 1
        if curNode.children:
            processNodes.extend(curNode.children)
...
def count_of_nodes(root, queries, s):
    ...
    for query in queries:
        u = query[0]
        c = query[1]
        node = findNode(root, u)
        res.append(trasverseTree(node, c, s))
